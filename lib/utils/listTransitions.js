...
export const deriveActivityVerb = ({ prev = {}, next }) => {
  const previousReview = typeof prev.review === 'string' ? prev.review.trim() : '';
  const nextReview = typeof next?.review === 'string' ? next.review.trim() : previousReview;
  if (nextReview && nextReview !== previousReview) return 'review';
  if (next?.status === 'completed' && prev.status !== 'completed') return 'complete';
  const previousRating = typeof prev.rating === 'number' ? prev.rating : null;
  if (next?.hasRating && typeof next?.rating === 'number' && next.rating !== previousRating) {
    return 'rate';
  }
  return 'watch';
};
