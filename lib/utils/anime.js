...
export const isReleased = (anime) => {
  const status = String(anime?.status || '').toLowerCase();
  if (status.includes('not yet aired')) return false;
  if (status === 'upcoming') return false;
  return true;
};

export const filterUnreleased = (items = []) => items.filter(isReleased);
...
export const classifyAnimeFormat = (anime) => {
  const rawType = String(anime?.type || '').toLowerCase();
  const eps = Number(anime?.episodesTotal ?? anime?.episodes ?? 0);
  if (rawType === 'movie') return { key: 'movie', label: 'MOVIE' };
  if (rawType === 'ova') return { key: 'ova', label: `OVA${eps ? ` / ${eps} EP` : ''}` };
  if (rawType === 'ona') return { key: 'ona', label: `ONA${eps ? ` / ${eps} EP` : ''}` };
  if (rawType === 'special') return { key: 'special', label: 'SPECIAL' };
  if (rawType === 'music') return { key: 'music', label: 'MUSIC' };
  if (Number.isFinite(eps) && eps > 0 && eps <= 13) {
    return { key: 'slice', label: `SLICE / ${eps} EP` };
  }
  if (Number.isFinite(eps) && eps > 13) {
    return { key: 'longform', label: `LONG-FORM / ${eps} EP` };
  }
  return { key: 'series', label: rawType ? rawType.toUpperCase() : 'SERIES' };
};
