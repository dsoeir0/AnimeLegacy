...
const OPTIMISTIC_READ_SKEW_MS = 2000;

export const mergeOptimisticState = (state, optimisticDismissed, optimisticReadAt) => {
  const dismissedExtras = Array.isArray(optimisticDismissed) ? optimisticDismissed : [];
  const hasOptimisticRead = optimisticReadAt !== null && optimisticReadAt !== undefined;
  if (dismissedExtras.length === 0 && !hasOptimisticRead) return state;
  return {
    ...state,
    lastReadAt: hasOptimisticRead ? optimisticReadAt : state?.lastReadAt ?? null,
    dismissedIds: dismissedExtras.length
      ? [...(state?.dismissedIds || []), ...dismissedExtras]
      : state?.dismissedIds,
  };
};

export const shouldClearOptimisticReadAt = (stateLastReadAt, optimisticReadAt) => {
  if (optimisticReadAt === null || optimisticReadAt === undefined) return false;
  if (!Number.isFinite(stateLastReadAt)) return false;
  return stateLastReadAt >= optimisticReadAt - OPTIMISTIC_READ_SKEW_MS;
};

export const filterUnconfirmedDismissed = (optimisticDismissed, serverDismissedIds) => {
  const optimistic = Array.isArray(optimisticDismissed) ? optimisticDismissed : [];
  if (optimistic.length === 0) return optimistic;
  const serverIds = new Set(Array.isArray(serverDismissedIds) ? serverDismissedIds : []);
  return optimistic.filter((id) => !serverIds.has(id));
};

export const buildSeenPatchFromNotifications = (notifications) => {
  const patch = {};
  if (!Array.isArray(notifications)) return patch;
  for (const n of notifications) {
    if (n?.kind !== 'newEpisode') continue;
    const count = n?.extra?.episodes;
    if (count === null || count === undefined) continue;
    const malId = n?.malId;
    if (malId === null || malId === undefined) continue;
    patch[String(malId)] = Number(count);
  }
  return patch;
};
